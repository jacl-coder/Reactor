...
EventLoop::EventLoop(bool mainLoop, int timetvl, int timeout)
    : ep_(new Epoll), mainLoop_(mainLoop), timetvl_(timetvl), timeout_(timeout),
      ...
      timerfd_(createtimefd(timeout_)), timerchannel_(new Channel(this, timerfd_))
{
...
void EventLoop::run()
{
    ...
    while (true)
    {
        std::vector<Channel *> channels = ep_->loop(10 * 1000); // 10s
...
void EventLoop::handletimer()
{
    ...
    if (mainLoop_)
    {
        // printf("EventLoop::handletimer() main loop thread is: %ld\n", syscall(SYS_gettid));
    }
    else
    {
        printf("EventLoop::handletimer() not main loop thread is: %ld\n", syscall(SYS_gettid));
